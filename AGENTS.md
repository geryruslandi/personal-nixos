...
- **Plugin-owned services**: seanime, stremio, and mailpit are declared and managed **by their Noctalia plugins** (`home-sync/.local/share/noctalia/plugins/<name>`) as transient systemd **user** units (`systemd-run --user --unit=<name> --collect --property=Restart=on-failure`); Nix only installs the binary dependency (`home-modules/seanime.nix`, `stremio.nix`, `mailpit.nix`). `secrets.server.*.enable` feeds the plugin's `auto_start` setting (`plugin_settings` in `home-modules/noctalia.nix`). **Warp and seaweedfs stay Nix-declared** — they need root daemon / system user + tmpfiles — the plugins only toggle them.
