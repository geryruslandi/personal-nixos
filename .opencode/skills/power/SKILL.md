...
| 7 | On the **greeter** (Noctalia Greeter via greetd), a real "Lid closed" after a rescue-resume can be **eaten** by logind (evdev lid stream desyncs) → closing the lid does nothing | **`lid-watch` timer** (every ~15s): only when the active session class is **greeter** (never a user session), polls `/proc/acpi/button/lid/LID0/state`; if closed and no suspend in flight → `systemctl suspend`. Shares the 5-attempt guard | `lid-watch` |
...
- **Greeter**: after a rescue-resume the Noctalia Wayland greeter can fail to re-apply outputs (`drmModeListLessees() failed`) → black screen that looks like "not awake". The guard + `lid-watch` re-sleep within seconds/15s; a genuine lid open afterwards resumes a clean greeter.
